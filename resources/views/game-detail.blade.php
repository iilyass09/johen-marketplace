...
<section class="game-hero">
  <div class="game-hero-bg"@if($brand->thumbnail_url) style="background-image:url('{{ $brand->thumbnail_url }}')"@endif></div>
  ...
  <div class="game-hero-inner">
    <div class="game-hero-icon">
      ...
      @else
        <span style="font-size:2rem">{{ $brand->icon ?? '🎮' }}</span>
      @endif
    </div>
...
<div class="gd-wrap">
  ...
  <div class="gd-step">
    ...
    <div class="gd-pay-group" id="payGroup"></div>
  </div>
  ...
  <div class="gd-step">
    ...
    <div class="gd-card">
      <label for="promoInput" class="gd-promo-label">Masukan Kode Promo Kamu</label>
      <div class="gd-promo-row">
        <input type="text" id="promoInput" placeholder="JOHENI10">
        <button class="btn btn-outline" id="promoBtn">Terapkan</button>
      </div>
...
function payRowHTML(m) {
    return `<div class="gd-pay-row" data-key="${m.key}">
      <button type="button" class="gd-pay-row-head" data-key="${m.key}">
        <span class="gd-pay-icon">${m.photo ? `<img src="${m.photo}" alt="${m.title}" class="pay-badge-img">` : '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>'}</span>
        <span class="gd-pay-label"><span class="gd-pay-t">${m.title}</span></span>
...
$('#payGroup').innerHTML = paymentMethods.map(payRowHTML).join('');

if (paymentMethods.length) {
    selectedPayFee = paymentMethods[0].fee || 0;
    selectedPayLabel = paymentMethods[0].title;
    $(`.gd-pay-row[data-key="${paymentMethods[0].key}"]`).classList.add('selected');
}
